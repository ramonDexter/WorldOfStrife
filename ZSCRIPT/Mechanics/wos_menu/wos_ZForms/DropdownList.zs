...
class wos_ZF_DropdownList : wos_ZF_Element {
	private void config(
		wos_ZF_DropdownItems items, Font fnt = NULL, double textScale = 1.0, int textColor = Font.CR_WHITE,
		wos_ZF_BoxTextures boxBg = NULL, wos_ZF_BoxTextures listBg = NULL, wos_ZF_BoxTextures highlightBg = NULL,
		string dropTex = "", int defaultSelection = -1, wos_ZF_Handler cmdHandler = NULL, Name command = '',
		wos_ZF_Frame bindingFrame = NULL
	) {
		setFont(fnt);
		setItems(items);
	...
	/// the dropdown choice.
	static wos_ZF_DropdownList create(
		...
		let ret = new("wos_ZF_DropdownList");
	...
	private wos_ZF_DropdownItems items;
	/// Returns the dropdown items list that this element uses.
	wos_ZF_DropdownItems getItems() { return self.items; }
	/// Sets the dropdown items list that this element uses.
	void setItems(wos_ZF_DropdownItems items) { self.items = items; }
	...
	private wos_ZF_Frame bindingFrame;
	...
	wos_ZF_Frame getBindingFrame() { return self.bindingFrame; }
	/// Sets the frame that binds this dropdown list.
	/// 
	/// If a dropdown list cannot fit inside of the `bindingFrame` by going
	/// downwards, it will instead go upwards. If `bindingFrame == NULL`, then
	/// the entire screen is effectively used as a binding frame.
	void setBindingFrame(wos_ZF_Frame bindingFrame) { self.bindingFrame = bindingFrame; }

	private wos_ZF_BoxTextures boxBg;
	/// Returns the box textures used for drawing the part of the element that
	/// is clicked on to drop down the list.
	wos_ZF_BoxTextures getBoxBgTexture() { return self.boxBg; }
	/// Sets the box textures used for drawing the part of the element that is
	/// clicked on to drop down the list.
	void setBoxBgTexture(wos_ZF_BoxTextures boxBg) { self.boxBg = boxBg; }
	private wos_ZF_BoxDrawer boxBgDrawer;

	private wos_ZF_BoxTextures listBg;
	/// Returns the box textures used for drawing the list part of the element
	/// that shows up when the element is clicked.
	wos_ZF_BoxTextures getListBgTexture() { return self.listBg; }
	/// Sets the box textures used for drawing the list part of the element
	/// that shows up when the element is clicked.
	void setListBgTexture(wos_ZF_BoxTextures listBg) { self.listBg = listBg; }
	private wos_ZF_BoxDrawer listBgDrawer;

	private wos_ZF_BoxTextures highlightBg;
	/// Returns the box textures used to draw the higlight for the current
	/// option of the user.
	wos_ZF_BoxTextures getHighlightBgTexture() { return self.highlightBg; }
	/// Sets the box textures used to draw the higlight for the current option
	/// of the user.
	void setHighlightBgTexture(wos_ZF_BoxTextures highlightBg) { self.highlightBg = highlightBg; }
	private wos_ZF_BoxDrawer highlightBgDrawer;
	...
	private void dropdownAABB(wos_ZF_AABB screenBox) {
		wos_ZF_AABB aabb;
		...
		if (bindingFrame != NULL) {
			wos_ZF_AABB bindingBox; bindingFrame.boxToScreen(bindingBox);
	...
	override bool blocksMousePriority(Vector2 mousePos) {
		...
		wos_ZF_AABB screenBox; dropdownAABB(screenBox);
	...
	override void drawer() {
		...
		wos_ZF_AABB beforeClip, clipRect;
	...
	private double getDropdownYOff() {
		...
		aabb.size = (box.size.x, box.size.y * items.items.size());
		wos_ZF_AABB dropdownBox; aabbToScreen(dropdownBox, aabb, false);
		wos_ZF_AABB masterBox;
		if (bindingFrame != NULL) {
	...
	private bool setSelectionFromMouse() {
		let mousePos = getGlobalStore().mousePos;
		wos_ZF_AABB dropdownBox; dropdownAABB(dropdownBox);
	...
	override bool onNavEvent(wos_ZF_NavEventType type, bool fromController) {
		if (isEnabled()) {
			if (active) {
				...
				return true;
			} else if (isFocused() && type == wos_ZF_NavEventType_Confirm) {
				active = true;
				setHoverBlock(self);
				hover = -1;
				return true;
	...
	override bool onUIEventPriority(wos_ZF_UiEvent ev) {
		let mousePos = getGlobalStore().mousePos;
	...
	override bool onUIEvent(wos_ZF_UiEvent ev) {
		if (ev.type == UIEvent.Type_LButtonDown) {
			if (isEnabled()) {
				wos_ZF_AABB screenBox; boxToScreen(screenBox);
				let mousePos = getGlobalStore().mousePos;

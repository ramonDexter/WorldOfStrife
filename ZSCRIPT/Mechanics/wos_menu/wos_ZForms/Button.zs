...
/// there.
class wos_ZF_Button : wos_ZF_ButtonBase {
	private void config(
		string text = "", wos_ZF_Handler cmdHandler = NULL, Name command = '',
		wos_ZF_BoxTextures inactive = NULL, wos_ZF_BoxTextures hover = NULL,
		wos_ZF_BoxTextures click = NULL, wos_ZF_BoxTextures disabled = NULL,
	...
	/// ticks.
	static wos_ZF_Button create(
		Vector2 pos, Vector2 size, string text = "", wos_ZF_Handler cmdHandler = NULL, Name command = '',
		wos_ZF_BoxTextures inactive = NULL, wos_ZF_BoxTextures hover = NULL, wos_ZF_BoxTextures click = NULL,
		wos_ZF_BoxTextures disabled = NULL, Font fnt = NULL, double textScale = 1, int textColor = Font.CR_WHITE,
		int holdInterval = -1, AlignType alignment = AlignType_Center
	) {
		let ret = new('wos_ZF_Button');
	...
	override bool onNavEvent(wos_ZF_NavEventType type, bool fromController) {
		if (isEnabled() && isFocused() && type == wos_ZF_NavEventType_Confirm && holdTicInterval == -1) {

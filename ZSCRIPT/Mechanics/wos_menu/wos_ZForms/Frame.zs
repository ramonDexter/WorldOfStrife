...
class wos_ZF_Frame : wos_ZF_ElementContainer {
	/// Returns a newly-created frame element.
	///
	/// The position used is relative to whatever the element is packed into later.
	static wos_ZF_Frame create(Vector2 pos, Vector2 size) {
		let ret = new('wos_ZF_Frame');
	...
	/// ?doc: hidden
	void internalPack(wos_ZF_Element elem) {
		if (elem.master != NULL) {
			elem.unpack();
		}
		elem.master = self;
	...
	/// ?doc: hidden
	void internalUnpack(wos_ZF_Element elem) {
		if (elem.master != self) {
	...
	virtual void onPack(wos_ZF_Element elem) {}

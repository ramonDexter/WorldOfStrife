...
class wos_ZF_Slider : wos_ZF_Element {
	private void config(
		double low, double high, double value, double step, double handleWidth,
		wos_ZF_BoxTextures handle, wos_ZF_BoxTextures handleHover, wos_ZF_BoxTextures handleClick = NULL,
		wos_ZF_BoxTextures background = NULL, wos_ZF_Handler cmdHandler = NULL, Name command = ''
	) {
		setLow(low);
		setHigh(high);
		setStep(step);
	...
	/// value of the slider.
	static wos_ZF_Slider create(
		Vector2 pos, Vector2 size,
		double low, double high, double value, double step, double handleWidth,
		wos_ZF_BoxTextures handle, wos_ZF_BoxTextures handleHover, wos_ZF_BoxTextures handleClick = NULL,
		wos_ZF_BoxTextures background = NULL, wos_ZF_Handler cmdHandler = NULL, Name command = ''
	) {
		let ret = new("wos_ZF_Slider");
	...
	private wos_ZF_BoxTextures background;
	/// Returns the box textures used for drawing the background of the slider.
	wos_ZF_BoxTextures getBackgroundTexture() { return self.background; }
	/// Sets the box textures used for drawing the background of the slider.
	void setBackgroundTexture(wos_ZF_BoxTextures background) { self.background = background; }
	private wos_ZF_BoxDrawer backgroundDrawer;
	...
	private wos_ZF_BoxTextures handle[3];

	/// Returns the box textures used when the slider is in the
	/// [normal](HandleState.Handle_Normal) state.
	wos_ZF_BoxTextures getHandleTexture() { return handle[Handle_Normal]; }
	/// Sets the box textures used when the slider is in the
	/// [normal](HandleState.Handle_Normal) state.
	void setHandleTexture(wos_ZF_BoxTextures tex) { handle[Handle_Normal] = tex; }

	/// Returns the box textures used when the slider is in the
	/// [hover](HandleState.Handle_Hover) state.
	wos_ZF_BoxTextures getHandleHoverTexture() { return handle[Handle_Hover]; }
	/// Sets the box textures used when the slider is in the
	/// [hover](HandleState.Handle_Hover) state.
	void setHandleHoverTexture(wos_ZF_BoxTextures tex) { handle[Handle_Hover] = (tex != NULL) ? tex : handle[Handle_Normal]; }
	
	/// Returns the box textures used when the slider is in the
	/// [click](HandleState.Handle_Click) state.
	wos_ZF_BoxTextures getHandleClickTexture() { return handle[Handle_Click]; }
	/// Sets the box textures used when the slider is in the
	/// [click](HandleState.Handle_Click) state.
	void setHandleClickTexture(wos_ZF_BoxTextures tex) { handle[Handle_Click] = (tex != NULL) ? tex : handle[Handle_Normal]; }

	private wos_ZF_BoxDrawer handleDrawer[3];

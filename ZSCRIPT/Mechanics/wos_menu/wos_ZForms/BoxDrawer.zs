...
struct wos_ZF_BoxDrawer ui {
	...
	private wos_ZF_BoxTextures lastTextures;
	private Vector2 lastScale;

	/// Draws the [`ZF_BoxTextures`] in the context of `master`, changing the cached information if
	/// necessary.
	void draw(
		wos_ZF_Element master, Vector2 pos, Vector2 size, wos_ZF_BoxTextures textures,
		...
		wos_ZF_AABB clipRect; master.getClipAABB(clipRect);
		Screen.drawShape(
	...
	private Shape2D calc(
		wos_ZF_Element master, Vector2 size, wos_ZF_BoxTextures textures, Vector2 scale = (1, 1)
	) {

...
class wos_ZF_BoxTextures ui {
	...
	static wos_ZF_BoxTextures createSingleTexture(string tex, bool scaleTex) {
		let ret = new('wos_ZF_BoxTextures');
	...
	static wos_ZF_BoxTextures createTexturePixels(
	...
	/// `scaleSides` and `scaleMiddle` being true make the side and middle textures (respectively)
	/// scale to grow larger or smaller, otherwise they will repeat to fit the space.
	static wos_ZF_BoxTextures createTextureNormalized(
		string boxTex, Vector2 topLeft, Vector2 bottomRight, bool scaleSides,
		bool scaleMiddle
	) {
		let ret = new('wos_ZF_BoxTextures');

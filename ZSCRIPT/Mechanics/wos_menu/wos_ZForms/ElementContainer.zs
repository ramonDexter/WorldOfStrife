...
class wos_ZF_ElementContainer : wos_ZF_Element {
	...
	/// it.
	protected Array<wos_ZF_Element> elements;
	...
	private void drawFocusIndicator(wos_ZF_ElementTreeGlobal globalStore) {
	...
	private void drawerInternal(bool top) {
		...
		wos_ZF_AABB beforeClip; getClipAABB(beforeClip);
		if (master != NULL) {
			wos_ZF_AABB clipCoords; boxToScreen(clipCoords);
		...
		if (!top) {
			if (master == NULL && globalStore.focusPriority == wos_ZF_FocusPriority_AboveAll) {

...
class wos_ZF_BoxImage : wos_ZF_Element {
	private void config(wos_ZF_BoxTextures textures = NULL, Vector2 scale = (1, 1)) {
		setBoxTex(textures);
		setBoxScale(scale);
		setAlpha(1.0);
	}
	...
	static wos_ZF_BoxImage create(Vector2 pos, Vector2 size, wos_ZF_BoxTextures textures, Vector2 scale = (1, 1)) {
		let ret = new('wos_ZF_BoxImage');

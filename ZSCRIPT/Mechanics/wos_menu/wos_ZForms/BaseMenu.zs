...
class wos_ZF_GenericMenu : GenericMenu {
	...
	wos_ZF_Element focusDefaults[wos_ZF_NavEventType_FocusChangeCount];

	private wos_ZF_ElementTreeGlobal globalStore;
	...
	/// direction `type`.
	void setFocus(wos_ZF_Element elem, wos_ZF_NavEventType type) {
	...
	/// focused.
	wos_ZF_Element getFocusIndicator() { return globalStore.focusIndicator; }
	/// Sets the element that is drawn as an indicator of what element is
	/// focused.
	void setFocusIndicator(wos_ZF_Element focusIndicator) { globalStore.focusIndicator = focusIndicator; }

	/// Returns the focus indicator's priority.
	///
	/// See [`ZF_FocusPriority`].
	wos_ZF_FocusPriority getFocusPriority() { return globalStore.focusPriority; }
	/// Sets the focus indicator's priority.
	///
	/// See [`ZF_FocusPriority`].
	void setFocusPriority(wos_ZF_FocusPriority focusPriority) { globalStore.focusPriority = focusPriority; }

	override void init(Menu parent) {
		Super.init(parent);
		mainFrame = wos_ZF_Frame.create((0, 0), (320, 200));
		globalStore = new("wos_ZF_ElementTreeGlobal");
	...
	virtual void changeFocusIndicator(wos_ZF_Element oldFocus, wos_ZF_Element newFocus) {}
	...
	private void fireNavigationEvent(wos_ZF_NavEventType type, bool fromController) {
		if (type == wos_ZF_NavEventType_Deny) {
			handleBack();
		}

		if (mainFrame.onNavEvent(type, fromController)) return;

		if (type < wos_ZF_NavEventType_FocusChangeCount) {
			wos_ZF_Element newFocus;
			if (globalStore.focus == NULL) {
	...
	override bool onUIEvent(UIEvent ev) {
		...
		wos_ZF_UiEvent.fromGZDUiEvent(ev, zfEv);
	...
	override bool menuEvent(int mkey, bool fromcontroller) {
		...
		switch (mkey) {
		case MKEY_Up:    fireNavigationEvent(wos_ZF_NavEventType_Up, fromcontroller); break;
		case MKEY_Down:  fireNavigationEvent(wos_ZF_NavEventType_Down, fromcontroller); break;
		case MKEY_Left:  fireNavigationEvent(wos_ZF_NavEventType_Left, fromcontroller); break;
		case MKEY_Right: fireNavigationEvent(wos_ZF_NavEventType_Right, fromcontroller); break;

		case MKEY_PageUp:   fireNavigationEvent(wos_ZF_NavEventType_PageUp, fromcontroller); break;
		case MKEY_PageDown: fireNavigationEvent(wos_ZF_NavEventType_PageDown, fromcontroller); break;

		case MKEY_Enter: fireNavigationEvent(wos_ZF_NavEventType_Confirm, fromcontroller); break;
		case MKEY_Back:  fireNavigationEvent(wos_ZF_NavEventType_Deny, fromcontroller); break;
		}
		return true;
	}
}
...
//  properly anyway
/// ?doc: hidden
class wos_ZF_EventHandlerMenu ui {
	wos_ZF_Frame mainFrame;
	...
	void copyCustomMousePosition(wos_ZF_EventHandlerMenu other) {

...
class wos_ZF_ElementTreeGlobal {
...
enum wos_ZF_NavEventType {
	...
	wos_ZF_NavEventType_FocusChangeCount,

	/// Typically caused by the Page Up key.
	wos_ZF_NavEventType_PageUp = wos_ZF_NavEventType_FocusChangeCount,
	/// Typically caused by the Page Down key.
	wos_ZF_NavEventType_PageDown,

	/// Typically caused by the Enter key, A button, etc.
	wos_ZF_NavEventType_Confirm,
	/// Typically caused by the Escape key, B button, etc.
	wos_ZF_NavEventType_Deny
}
...
class wos_ZF_Element ui {
	...
	protected wos_ZF_Element master;

	/// ?doc: hidden
	protected wos_ZF_ElementTreeGlobal globalStore;
	/// ?doc: hidden
	void setGlobalStore(wos_ZF_ElementTreeGlobal globalStore) {
		if (master == NULL) {
	...
	protected wos_ZF_Element focusNeighbors[wos_ZF_NavEventType_FocusChangeCount];
	/// Returns the neighbor in the direction `dir` used for changing focus with the keyboard/game
	/// controller.
	wos_ZF_Element getFocusNeighbor(wos_ZF_NavEventType dir) {
	...
	/// [`ZF_FocusLinkHelper`].
	void setFocusNeighbor(wos_ZF_NavEventType dir, wos_ZF_Element neighbor) {
	...
	protected void setHoverBlock(wos_ZF_Element val) {
		let s = getGlobalStore();
		if (s != NULL) {
			s.hoverBlock = val;
		}
	...
	/// ?doc: hidden
	void getClipAABB(wos_ZF_AABB curClip) {
	...
	/// entire tree.
	protected wos_ZF_ElementTreeGlobal getGlobalStore() {
	...
	/// visible.
	void aabbToScreen(wos_ZF_AABB ret, wos_ZF_AABB bounds, bool intersect = true) {
		ret.pos = relToScreen(bounds.pos) * getScale();
		ret.size = bounds.size * getScale();

		if (intersect && master != NULL) {
			wos_ZF_AABB screenBox; master.boxToScreen(screenBox, true);
	...
	/// higher in the element tree to avoid this AABB being considered where it's not actually
	/// visible.
	void boxToScreen(wos_ZF_AABB ret, bool intersect = true) {
	...
	/// box is used.
	void screenClip(wos_ZF_AABB beforeClip, wos_ZF_AABB clipRect, wos_ZF_AABB aabb = NULL) {
	...
	void drawLine(Vector2 start, Vector2 end, double thickness, Color color) {
		...
		wos_ZF_AABB aabb; getClipAABB(aabb);
		bool draw;
	...
	void drawImage(Vector2 relPos, string imageName, bool animate, Vector2 scale = (1, 1), double alpha = 1, wos_ZF_AABB clipRect = NULL) {
	...
	void drawTiledImage(Vector2 relPos, Vector2 size, string imageName, bool animate, Vector2 scale = (1, 1), double alpha = 1.0) {
		...
		wos_ZF_AABB clipRect; getClipAABB(clipRect);
	...
	/// element is focused.
	protected void drawSubElement(wos_ZF_Element elem) {
		let globalStore = getGlobalStore();
		if (globalStore.focusPriority == wos_ZF_FocusPriority_JustBelowFocused && elem == globalStore.focus) {
			drawFocusIndicator(globalStore);
		}
		elem.drawer();
		if (globalStore.focusPriority == wos_ZF_FocusPriority_JustAboveFocused && elem == globalStore.focus) {
			drawFocusIndicator(globalStore);
		}
	}
	...
	/// ?doc: hidden
	protected void drawFocusIndicator(wos_ZF_ElementTreeGlobal globalStore) {
		if (globalStore.focusIndicator == NULL) return;

		wos_ZF_AABB beforeClip;
		getClipAABB(beforeClip);
	...
	virtual bool onNavEvent(wos_ZF_NavEventType type, bool fromController) { return false; }
	/// Gets called when a UI event occurs.
	///
	/// Returning `true` blocks other elements in the tree after this one from receiving this.
	virtual bool onUIEvent(wos_ZF_UiEvent ev) { return false; }
	/// Gets called when a UI event occurs, before [`onUIEvent`] ever is.
	///
	/// Returning `true` blocks other elements in the tree after this one from receiving this.
	virtual bool onUIEventPriority(wos_ZF_UiEvent ev) { return false; }
	...
	virtual bool blocksMouse(Vector2 mousePos) {
		...
		wos_ZF_AABB screenBox; boxToScreen(screenBox);
	...
	virtual void getFocusAABB(wos_ZF_AABB box) { box.pos = relToMainFrame((0, 0)); box.size = self.box.size; }
	/// Gets called when the element has been put in focus by a navigation event.
	virtual void beenFocused(wos_ZF_NavEventType type) {}

...
/// [`ZF_TextInput`].
class wos_ZF_Label : wos_ZF_Element {
	private void config(
		...
		wos_ZF_Element forElement = NULL
	) {
		calculate = false;
		setFont(fnt);
		setText(text);
	...
	static wos_ZF_Label create(
		...
		double lineSpacing = 0, wos_ZF_Element forElement = NULL
	) {
		let ret = new('wos_ZF_Label');
	...
	private wos_ZF_Element forElement;
	...
	wos_ZF_Element getForElement() { return self.forElement; }
	...
	void setForElement(wos_ZF_Element forElement) { self.forElement = forElement; }
	...
	override bool onNavEvent(wos_ZF_NavEventType type, bool fromController) {
		...
		if (isFocused() && type == wos_ZF_NavEventType_Confirm) {

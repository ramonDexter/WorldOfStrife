...
/// draw a scrollbar.
class wos_ZF_Scrollbar : wos_ZF_Element {
	/// Reconfigures the scrollbar. See [`create`] for argument details.
	void config(
		wos_ZF_BoxTextures normal, wos_ZF_BoxTextures hover = NULL,
		wos_ZF_BoxTextures click = NULL, wos_ZF_BoxTextures background = NULL
	) {
	...
	static wos_ZF_Scrollbar create(
		Vector2 pos, Vector2 size,
		wos_ZF_BoxTextures normal, wos_ZF_BoxTextures hover = NULL,
		wos_ZF_BoxTextures click = NULL, wos_ZF_BoxTextures background = NULL
	) {
		let ret = new('wos_ZF_Scrollbar');
	...
	/// The background used.
	wos_ZF_BoxTextures background;
	private wos_ZF_BoxDrawer bgDrawer;
	/// Textures drawn for each state in [`BarState`].
	wos_ZF_BoxTextures textures[3];
	private wos_ZF_BoxDrawer barDrawers[3];
	...
	private void barBox(wos_ZF_AABB ret) {
	...
	override bool handleMousePosition(bool mouseBlock, Vector2 mousePos) {
		if (curBarState == BarState_Click) {
			...
			wos_ZF_AABB box; barBox(box);
			wos_ZF_AABB screenBox; aabbToScreen(screenBox, box);

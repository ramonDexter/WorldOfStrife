...
class wos_ZF_TextInput : wos_ZF_Element {
	private void config(
		Font fnt = NULL, double textScale = 1, int textColor = Font.CR_WHITE,
		wos_ZF_BoxTextures background = NULL, bool multiline = false,
		wos_ZF_Handler cmdHandler = NULL, Name command = '',
		double scrollbarWidth = 0.0,
		wos_ZF_BoxTextures scrollBarNormal = NULL, wos_ZF_BoxTextures scrollBarHover = NULL,
		wos_ZF_BoxTextures scrollBarClick = NULL, wos_ZF_BoxTextures scrollBg = NULL
	) {
	...
	static wos_ZF_TextInput create(
		Vector2 pos, Vector2 size, Font fnt = NULL, double textScale = 1,
		int textColor = Font.CR_WHITE, wos_ZF_BoxTextures background = NULL,
		bool multiline = false, wos_ZF_Handler cmdHandler = NULL, Name command = '',
		double scrollbarWidth = 0.0,
		wos_ZF_BoxTextures scrollBarNormal = NULL, wos_ZF_BoxTextures scrollBarHover = NULL,
		wos_ZF_BoxTextures scrollBarClick = NULL, wos_ZF_BoxTextures scrollBg = NULL
	) {

		let ret = new('wos_ZF_TextInput');

		ret.setBox(pos, size);
		ret.bar = wos_ZF_Scrollbar.create((0, 0), (0, 0), NULL, NULL);
		ret.bar.master = ret;
	...
	private wos_ZF_BoxDrawer bgDrawer;
	...
	override bool onNavEvent(wos_ZF_NavEventType type, bool fromController) {
		if (isEnabled() && isFocused() && type == wos_ZF_NavEventType_Confirm) {
			setHoverBlock(self);
			getGlobalStore().blockMenuEvent = true;
			typing = true;
			blink = true;

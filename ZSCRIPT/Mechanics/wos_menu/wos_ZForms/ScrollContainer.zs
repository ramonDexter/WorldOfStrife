...
/// [frame](ZF_Frame).
class wos_ZF_ScrollContainer : wos_ZF_ElementContainer {
	private void config(
		double scrollWidth, double scrollAreaHeight, double scrollNotchJump,
		wos_ZF_BoxTextures scrollBarNormal, wos_ZF_BoxTextures scrollBarHover = NULL,
	...
	static wos_ZF_ScrollContainer create(
		Vector2 pos, Vector2 size,
		double scrollWidth, double scrollAreaHeight, double scrollNotchJump,
		wos_ZF_BoxTextures scrollBarNormal, wos_ZF_BoxTextures scrollBarHover = NULL,
		wos_ZF_BoxTextures scrollBarClick = NULL, wos_ZF_BoxTextures scrollBg = NULL,
		wos_ZF_Element scrollArea = NULL, double contentBarGap = 0.0
	) {
		let ret = new('wos_ZF_ScrollContainer');

		ret.setBox(pos, size);

		if (scrollArea == NULL) {
			ret.scrollArea = wos_ZF_Frame.create((0, 0), (0, 0));
		}
		else {
	...
	private wos_ZF_Element scrollArea;
	/// Returns the wrapped scroll area element so that it can be read from or
	/// modified.
	wos_ZF_Element getScrollArea() { return scrollArea; }

...
class wos_ZF_Handler ui {
	...
	virtual void buttonClickCommand(wos_ZF_Button caller, Name command) {}
	...
	virtual void buttonHeldCommand(wos_ZF_Button caller, Name command) {}
	...
	/// from the caller.
	virtual void radioButtonChanged(wos_ZF_RadioButton caller, Name command, wos_ZF_RadioController variable) {}

	/// Called whenever a [toggle button](ZF_ToggleButton) changes its state.
	virtual void toggleButtonChanged(wos_ZF_ToggleButton caller, Name command, bool on) {}

	/// Called whenever a [text input](ZF_TextInput) is exited from via any means.
	virtual void textInputExit(wos_ZF_TextInput caller, Name command) {}

	/// Called whenever the text inside a [text input](ZF_TextInput) is changed by the user.
	virtual void textInputChanged(wos_ZF_TextInput caller, Name command) {}

	/// Called whenever a [slider](ZF_Slider) is moved by the user.
	///
	/// The value should be gotten via [`ZF_Slider.getValue`].
	virtual void sliderChanged(wos_ZF_Slider caller, Name command) {}

	/// Called when a [dropdown list](ZF_DropdownList)'s item is changed by the
	/// user.
	virtual void dropdownChanged(wos_ZF_DropdownList caller, Name command) {}

	/// Called when any element is hovered or unhovered.
	virtual void elementHoverChanged(wos_ZF_Element caller, Name command, bool unhovered) {}
}

...
/// possibility.
class wos_ZF_FocusLinkHelper ui {
	private Array<wos_ZF_Element> elements;
	private uint width;
	private uint height;
	...
	static wos_ZF_FocusLinkHelper create(uint width, uint height, bool horzWraparound, bool vertWraparound) {
		let ret = new("wos_ZF_FocusLinkHelper");
	...
	void setElement(uint row, uint col, wos_ZF_Element elem) {
	...
	private void setUp(uint row, uint col) {
		...
		elements[row * width + col].setFocusNeighbor(wos_ZF_NavEventType_Up, elements[newRow * width + col]);
	}
	private void setDown(uint row, uint col) {
		...
		elements[row * width + col].setFocusNeighbor(wos_ZF_NavEventType_Down, elements[newRow * width + col]);
	}
	private void setLeft(uint row, uint col) {
		...
		elements[row * width + col].setFocusNeighbor(wos_ZF_NavEventType_Left, elements[row * width + newCol]);
	}
	private void setRight(uint row, uint col) {
		...
		elements[row * width + col].setFocusNeighbor(wos_ZF_NavEventType_Right, elements[row * width + newCol]);
	}

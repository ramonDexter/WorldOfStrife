////////////////////////////////////////////////////////////////////////////////
// by 4page
// https://github.com/4pageOfficial/ZScript_Boid_Library
////////////////////////////////////////////////////////////////////////////////


Struct HXA_BoidArray {
	Vector3 RandomMove[30];
	Array<int> FindNew;
}
...
Class HXA_Boid : ACTOR {
	Double VelocityMax;
	HXA_BoidArray BoidArray;
	Vector3 Beacon;
	int8 BeaconCounter;
	Bool CloseToMaster;
	Bool CloseToTarget;
	...
	Default {
		...
		Speed 0.1;
		radius 10;
		HXA_Boid.BoidActor "HXA_Boid";
		HXA_Boid.MaxVelocity 20;
		HXA_Boid.VelocityAcceleration .2;
		HXA_Boid.WallDetectionDistance 6;
		HXA_Boid.DistanceFromMaster 400;
		HXA_Boid.DistanceFromTarget 400;
		HXA_Boid.MinDistanceToBoid 150;
		HXA_Boid.MaxDistanceToBoid 900;
		HXA_Boid.BoidCohesion 3;
		HXA_Boid.MinDistanceBeacon 100;
		HXA_Boid.MaxDistanceBeacon 300;
		HXA_Boid.MaxAngleBeacon 100;
		HXA_Boid.MaxRandomMoveAngle 120;
		HXA_Boid.HorizonNormalizer  1;
		HXA_Boid.FlockID 0;
	}
	States {
		Spawn:
...
////////////////////////////////////////////////////////////////////////////////
	...
	int segregate(out HXA_BoidArray BoidArray, int size) {
		int j = 0;
		int i;
		for (i = 0; i < size; i++) {
	...
	int findMissingPositive(out HXA_BoidArray BoidArray, int size, int Start) {
		int i = Start;
...
////////////////////////////////////////////////////////////////////////////////
	...
	void NewFlockID() { //Really only useful for spawners.  Will give itself a new unused FlockID.	
		let BoidFinder = ThinkerIterator.Create("HXA_Boid", STAT_DEFAULT);
		HXA_Boid boid;
		while (true) {
			boid = HXA_Boid(BoidFinder.next());
...
// 		Console.printf(string.format("The new FlockID is %f",FlockID));
		Return;
	}
...
// 	The actor really needs to stay in motion, so it needs to start in motion if the boid movement is going to work.
	Override void PostBeginPlay() {	
// 		A_FaceMovementDirection();
// 		Populates the array to avoid issues with zero length vectors
// 		for(int i=0; i<BoidArray.RandomMove.size(); i++)
// 		{
// 			BoidArray.RandomMove[i] = vel;
// 		}
		Super.PostBeginPlay();
	}
...
//	This moves elements of an array from a HXA_BoidArray struct down one position, and adds a value to the beginning
	Action void ShiftArray(Vector3 AddTo, out HXA_BoidArray BoidArray) {
...
//	The properties will be used if the default arguments aren't changed unless UsePropertyValues is FALSE.
	Action Void BoidFlight(
		...
		if(UsePropertyValues) {
			BoidActor = BoidActor is "HXA_Boid" ? invoker.BoidActor : BoidActor;
		...
		bool AtSide;
		for(int i=0; i<30; i++) {
			vector3 FibVec = FibSphereDirection(i, 70); //Checks half a sphere centered on its velocity vector
		...
		if(InFront || AtSide) {
			...
			While(AVGvec.length() == 0) {
				breaker++;
				if(breaker > 100) { //So it doesn't get stuck in a permanent loop
					break;
				}
				for(int i=0; i<90; i++) {
					vector3 FibVec = FibSphereDirection(i, 90);//Checks in a full sphere
				...
				invoker.VelocityMax *= .9;	//Slows it down a bit near walls
			}
		...
		if(CloseToMaster && (!Target || !CloseToTarget) && Master && distance3d(Master) > DistanceFromMaster && GetAge()%3==0) {
			...
			double pitchtotarg = -Asin(ToTarget.z/ToTarget.length());
		...
		HXA_BoidArray BoidFinderArray;
		HXA_BoidArray BoidVelArray;
		...
		while (true) {
			...
			if(Boid == self || vecangletovec(vel,Vec) > 90 || Boid.FlockID != invoker.FlockID) {
				Continue;
			}
			Found = TRUE;
		...
		if(!InFront && !AtSide) { //If there's no walls nearby it will add a random vector to the average array
			...
			/* Vector3 UnitRPivRoll = RPivRoll.unit();
			Double PivRollPitch = -ASin(UnitRPivRoll.z);
			if(abs(VelP)>45 && abs(VelP+PivRollPitch) > Abs(VelP))
			{
				RPivRoll.z *= -1;
			} */
		...
		if(GetAge() > 5) {	
			...
			Vel = TurnTowardVec(Vel, AverageVector, vecangletovec(Vel,AverageVector)*.6);
			A_FaceMovementDirection();
		}
		for(int i=0; i<invoker.BoidArray.RandomMove.Size(); i++) {
	...
	action Actor HXA_FINDER(double maxDistance = 512, Bool RandomTargets = TRUE, 
		...
		if(Targets.Size() > 0 && Setter) {
			...
			if(RandomTargets) {
				...
				for(int i = 0; i < Targets.Size(); i++) {
					if(Distance3d(Targets[i]) < target.Distance3d(Targets[Selected])) { Selected = i; }
                }
            }
